...
class PromptedTextStyleTransferReward(BaseReward):
    def __init__(
        ...
        self.num_hypos = num_samples * num_bootstraps
    ...
    def compute_rewards(
        ...
        def compute_reward(prompt, source, label):
            def boostrap_max_rewards_k_times(
                    rewards: List[float],
                    k: int
            ) -> List[float]:
            ...
            def log_metrics():
                ...
                max_reward = max(bootstrapped_max_rewards)
                ...
                print(self._counter, '|',
                      ...
                      'Reward:', round(averaged_bootstrapped_max_reward.item(), 2), '|',
                      '# Explored Tokens', num_tokens_explored
                      )

            hypos = self._generator.sample_generate(prompt, source, self.num_hypos, self.top_k, self.top_p)
            sum_rewards, content_scores, style_probs = \
                self.selector.compute_sample_rewards(source, hypos, label)

            # Bootstrap the max reward for k times
            bootstrapped_max_rewards: List[float] = boostrap_max_rewards_k_times(sum_rewards, self.num_bootstraps)

            # Keep track of each input's max rewards to compute z-score
            self.input_rewards_per_batch[source] += bootstrapped_max_rewards

            # Average boostrap max rewards as the final reward
            averaged_bootstrapped_max_reward = torch.Tensor(bootstrapped_max_rewards).float().mean()

            log_metrics()

            return averaged_bootstrapped_max_reward
